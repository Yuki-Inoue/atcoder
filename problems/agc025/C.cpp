...
class Ranges {
    multiset<LR, LeftCompare> lefts;
    multiset<LR, RightCompare> rights;
...
public:
    ...
    LR pop_left() {
        //        cerr << "popping left" << endl;
        auto first = lefts.begin();
        LR ret = *first;
        lefts.erase(first);
        rights.erase(rights.find(ret));
        return ret;
    }

    LR pop_right() {
        //        cerr << "popping right" << endl;
        auto first = rights.begin();
        LR ret = *first;
        rights.erase(first);
        auto left_found = lefts.find(ret);
        // cerr << "left: " << *left_found << endl;
        // cerr << "ret: " << ret << endl;
        lefts.erase(lefts.find(ret));
        return ret;
    }

    LR pop(bool left) {
        return left ? pop_left() : pop_right();
    }
